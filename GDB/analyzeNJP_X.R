...
# Read Columb Matrices
nDim <- 23; nTot <- (nDim*(nDim+1)/2)
colnamesCM <- paste0("px", 1:nTot)
matrices <- read.csv("coulombX.csv", header = FALSE)
...
# Loop over parameters to find the best model
cv.nround = 1000
cv.nfold = 5
...
save(cv.results, file = "cvResultsX.RData")
...
# RMSE
sqrt(mean((pred - test.Y)^2))*eV_to_kcalMol # 11.0127 kcal/mol
